  ...
  try {
    const record = await prisma.notificationLog.create({
      data: {
        ...
        preferenceSnapshot: payload.preferenceSnapshot ? sanitizeMetadata(payload.preferenceSnapshot) : null,
        metadata: payload.metadata ? sanitizeMetadata(payload.metadata) : null,
